...
export class RouteManager {
     ...
     */
	public constructor(routePath: string, spec: OpenAPIV3.Document, middleware?: RequestHandler[]) {
	...
	public async loadRoutes(): Promise<void> {
        ...
        for await (const file of this.recursiveReadDir(this.routePath)) {
            ...
            if (instance.options.spec) this.spec.paths[instance.options.path] = instance.options.spec;
            this.routes.push(instance);
        }
        // Add callback for /openapi.json
        this.app.get('/openapi.json', (req, res) => res.status(200).json(this.spec));
    }
